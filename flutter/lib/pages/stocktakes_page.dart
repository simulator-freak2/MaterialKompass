...
class _StocktakesPageState extends State<StocktakesPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final responses = await Future.wait([
        ...
        http.get(
          Uri.parse('$apiBaseUrl/api/stocktakes/options'),
          headers: headers,
        ),
        http.get(Uri.parse('$apiBaseUrl/api/auth/me'), headers: headers),
        http.get(
          Uri.parse('$apiBaseUrl/api/stocktake-email-imports'),
          headers: headers,
        ),
      ]);
  ...
  Future<void> _assignEmail(Map<String, dynamic> email) async {
    String? selectedId = stocktakes.isEmpty
        ? null
        : stocktakes.first['id']?.toString();
      ...
      children: [
        ...
        Wrap(
          ...
          children: values.map((entry) {
            ...
            return FilterChip(
              label: Text(
                entry['path']?.toString() ?? entry['name']?.toString() ?? value,
              ),
...
class _StocktakeDetailPageState extends State<StocktakeDetailPage> {
  ...
  List<Map<String, dynamic>> get entries {
    final source = (stocktake?['entries'] as List? ?? const []).map(
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          actions: [
            ...
            FilledButton(
              ...
              child: Text(action == 'complete' ? 'Abschließen' : 'Bestätigen'),
            ),
          ],
        ),
      ),
  ...
  Future<void> _count(Map<String, dynamic> entry) async {
    ...
    final quantity = TextEditingController(
      text: entry['actualQuantity']?.toString() ?? '',
    );
    final notes = TextEditingController(text: entry['notes']?.toString() ?? '');
    String result = entry['result']?.toString() ?? 'vorhanden';
    String? locationId =
        entry['actualLocationId']?.toString() ??
        entry['expectedLocationId']?.toString();
    String? stockId =
        entry['actualStockStructureId']?.toString() ??
        entry['expectedStockStructureId']?.toString();
    final locations = (widget.options['locations'] as List? ?? const [])
        .cast<Map>();
    final stocks = (widget.options['stockStructures'] as List? ?? const [])
        .cast<Map>();
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      initialValue:
                          locationStocks.any((stock) => stock['id'] == stockId)
                          ? stockId
                          : null,
                      decoration: const InputDecoration(
                ...
                onPressed: () {
                  if (bulk &&
                      double.tryParse(quantity.text.replaceAll(',', '.')) ==
                          null) {
                    message(
                      'Bitte eine gültige Ist-Menge eingeben.',
                      error: true,
                    );
                    return;
                  }
  ...
  Future<void> _import() async {
    ...
    final data = await request(
      '/api/stocktakes/${widget.stocktakeId}/import',
      method: 'POST',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(data['name']?.toString() ?? 'Inventur'),
        actions: [
          if (can('stocktakes.export'))
            PopupMenuButton<String>(
              ...
              itemBuilder: (_) => const [
                PopupMenuItem(
                  value: 'pdf:blank',
                  child: Text('Zählliste als PDF'),
                ),
                PopupMenuItem(
                  value: 'xlsx:blank',
                  child: Text('Zählliste als XLSX'),
                ),
                PopupMenuItem(
                  value: 'ods:blank',
                  child: Text('Zählliste als ODS'),
                ),
                PopupMenuDivider(),
                PopupMenuItem(value: 'pdf', child: Text('Ergebnis als PDF')),
                PopupMenuItem(value: 'xlsx', child: Text('Ergebnis als XLSX')),
                PopupMenuItem(value: 'ods', child: Text('Ergebnis als ODS')),
                PopupMenuDivider(),
                PopupMenuItem(
          ...
          children: [
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    LinearProgressIndicator(
                      value:
                          (num.tryParse(progress['total']?.toString() ?? '') ??
                                  0) ==
                              0
                          ? 0
                          : (num.tryParse(
                    ...
                    Wrap(
                      ...
                      runSpacing: 8,
                      children: [
                        if (status == 'Angelegt' && can('stocktakes.count'))
                          FilledButton.icon(
                            onPressed: () => _transition('start'),
                            icon: const Icon(Icons.play_arrow),
                            label: const Text('Inventur starten'),
                          ),
                        if (status == 'In Arbeit' && can('stocktakes.evaluate'))
                          FilledButton.icon(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    items: const ['Alle', 'Offen', 'Gezählt', 'Abweichung']
                        .map(
                ...
                child: ListTile(
                  ...
                  subtitle: Text(
                    '${entry['expectedLocationName'] ?? '-'} / ${entry['expectedStockStructureName'] ?? '-'}\n${counted
                        ? entry['itemType'] == 'bulk'
                              ? 'Ist: ${entry['actualQuantity']} ${entry['unit']}'
                              : 'Ergebnis: ${entry['result']}'
                        : 'Noch nicht gezählt'}${discrepancies.isNotEmpty ? ' · ${discrepancies.join(', ')}' : ''}${counted ? '\n${entry['countedByName']} · ${entry['attempts']?.length ?? 1}. Zählung' : ''}',
                  ),
                  isThreeLine: true,
            ...
            if (entries.isEmpty)
              const Padding(
                ...
                child: Center(child: Text('Keine passenden Positionen.')),
              ),
            ...
            if (data['method'] == 'offline') ...[
              ...
              if (emailImports.isEmpty)
                const ListTile(
                  ...
                  title: Text('Noch keine Liste per E-Mail eingegangen.'),
                ),
              ...emailImports.map(
                (email) => Card(
                  child: ExpansionTile(
                    ...
                    children: [
                      ...(email['attachments'] as List? ?? const []).map(
                        (attachment) => ListTile(
                          leading: const Icon(Icons.attach_file),
                          title: Text(attachment['fileName'].toString()),
                          subtitle: Text(
