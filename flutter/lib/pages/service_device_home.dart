...
class _ServiceDeviceHomePageState extends State<ServiceDeviceHomePage> {
          ...
          TextButton.icon(
            onPressed: () async {
              await Clipboard.setData(
                ClipboardData(
                  text: '${report['defectNumber']} · ${data['accessCode']}',
                ),
              );
            },
  ...
  Future<void> accessDefect() async {
    final credentials = await showDialog<Map<String, String>>(
      context: context,
      builder: (_) => const _DefectAccessDialog(),
    );
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: Text('Systemzugang · ${widget.device['name'] ?? 'Dienstgerät'}'),
      actions: [
        IconButton(
          onPressed: logout,
          tooltip: 'Abmelden',
...
class _DeviceDefectDialogState extends State<_DeviceDefectDialog> {
  late final title = TextEditingController(
    text: widget.report?['title']?.toString(),
  );
  late final description = TextEditingController(
    text: widget.report?['description']?.toString(),
  );
  late final risk = TextEditingController(
    text: widget.report?['riskLevel']?.toString(),
  );
  late final measures = TextEditingController(
    text: widget.report?['measuresTaken']?.toString(),
  );
  late final location = TextEditingController(
    text:
        widget.report?['deviceLocation']?.toString() ?? widget.defaultLocation,
  );
  late final contactName = TextEditingController(
    text: widget.report?['contactName']?.toString(),
  );
  late final contactEmail = TextEditingController(
    text: widget.report?['contactEmail']?.toString(),
  );
  late final contactPhone = TextEditingController(
    text: widget.report?['contactPhone']?.toString(),
  );
  ...
  @override
  Widget build(BuildContext context) => AlertDialog(
    title: Text('${widget.item['name']} · ${widget.item['inventoryNumber']}'),
    content: SizedBox(
      width: 620,
