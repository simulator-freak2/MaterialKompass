...
Future<OfflineNetworkKind> offlineNetworkKind() async {
  final names = (await probe.activeNetworkNames()).map(
    (entry) => entry.toLowerCase(),
  );
  final unmetered = names.any(
    (name) => RegExp(
      r'wlan|wi-?fi|ethernet|^en\d|^eth\d|lan-verbindung|local area',
    ).hasMatch(name),
  );
  if (unmetered) return OfflineNetworkKind.unmetered;
  final mobile = names.any(
    (name) =>
