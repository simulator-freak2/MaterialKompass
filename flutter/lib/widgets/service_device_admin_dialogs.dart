      ...
      child: Column(
        ...
        children: [
          DropdownButtonFormField<String>(
            initialValue: userId,
            decoration: const InputDecoration(labelText: 'Persönliches Konto'),
            items: widget.users
                .where((entry) => entry['active'] == true)
                .map(
...
class _ServiceDeviceDialogState extends State<_ServiceDeviceDialog> {
  ...
  late final room = TextEditingController(
    text: widget.device?['room']?.toString(),
  );
  late final inventoryNumber = TextEditingController(
    text: widget.device?['inventoryNumber']?.toString(),
  );
  late final mac = TextEditingController(
    text: widget.device?['macAddress']?.toString(),
  );
  late final description = TextEditingController(
    text: widget.device?['description']?.toString(),
  );
  late final networks = TextEditingController(
    text: (widget.device?['allowedNetworks'] as List? ?? const []).join('\n'),
  );
  final password = TextEditingController();
  late String? locationId =
      widget.device?['locationId']?.toString() ??
      (widget.locations.isEmpty
          ? null
          : widget.locations.first['id'].toString());
  late String? responsibleUserId = widget.device?['responsibleUserId']
      ?.toString();
  ...
  Widget build(BuildContext context) => AlertDialog(
    title: Text(
      widget.device == null ? 'Dienstgerät anlegen' : 'Dienstgerät bearbeiten',
    ),
    content: SizedBox(
      width: 650,
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                helperText: 'Nur Dokumentation, kein Sicherheitsmerkmal',
              ),
            ),
            TextField(
            ...
            SwitchListTile(
              ...
              onChanged: (value) => setState(() => offlineEnabled = value),
            ),
            if (offlineEnabled) ...[
              Align(
                alignment: Alignment.centerLeft,
                child: Text(
                  'Offline berechtigte Benutzer',
                  style: Theme.of(context).textTheme.titleSmall,
                ),
              ),
              ...widget.users
              ...
              children: [
                Expanded(
                  child: mfaField(
                    'MFA Systemlogin',
                    systemMfa,
                    (value) => setState(() => systemMfa = value ?? 'off'),
                  ),
                ),
                ...
                Expanded(
                  child: mfaField(
                    ...
                    personalMfa,
                    (value) => setState(() => personalMfa = value ?? 'off'),
                  ),
                ),
              ],
            ),
    ...
    actions: [
      ...
      FilledButton(
        onPressed: () => Navigator.pop(context, {
          ...
          'active': active,
          if (password.text.isNotEmpty) 'systemPassword': password.text,
        }),
