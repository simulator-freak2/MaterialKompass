...
class _ServiceDevicesAdminPanelState extends State<ServiceDevicesAdminPanel> {
  ...
  Future<void> load() async {
    ...
    final responses = await Future.wait([
      AppHttpClient.get(Uri.parse('$apiBaseUrl/api/service-devices'),
          headers: headers),
      AppHttpClient.get(Uri.parse('$apiBaseUrl/api/locations'),
          headers: headers),
      AppHttpClient.get(Uri.parse('$apiBaseUrl/api/offline/clients'),
          headers: headers),
    ]);
    if (!mounted) return;
  ...
  Future<void> edit([Map<String, dynamic>? device]) async {
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      return message(object(response)['error']?.toString() ??
  ...
  Future<String?> textDialog(String title, String label,
  ...
  Future<void> addNfc(Map<String, dynamic> device) async {
    ...
    if (label == null) return;
    final credential =
        await textDialog('NFC-Karte lesen', 'USB-Kartenleser', secret: true);
  ...
  Future<void> addPersonalNfc(Map<String, dynamic> device) async {
    ...
    if (result == null) return;
    final response = await send(
        'POST', '/api/service-devices/${device['id']}/personal-nfc', result);
    if (!mounted) return;
    if (response.statusCode != 201) {
      return message(object(response)['error']?.toString() ??
  ...
  Future<void> issueOfflineQr(Map<String, dynamic> device) async {
    ...
    final users = widget.users
        .where((entry) =>
            entry['active'] == true && allowed.contains(entry['id'].toString()))
        .toList();
    if (users.isEmpty) {
      return message(
  ...
  Future<void> addTotp(Map<String, dynamic> device) async {
    final label =
        await textDialog('TOTP-Berechtigung anlegen', 'Verantwortliche Person');
    if (label == null) return;
    final response = await send('POST',
        '/api/service-devices/${device['id']}/totp-factors', {'label': label});
    if (!mounted) return;
    final data = object(response);
    if (response.statusCode != 201) {
      return message(
  ...
  Future<void> action(
      Map<String, dynamic> device, String action, String success) async {
    final response =
        await send('POST', '/api/service-devices/${device['id']}/$action');
    if (!mounted) return;
    if (response.statusCode != 200) {
      return message(
          ...
          child: devices.isEmpty
              ...
              : ListView(
                  ...
                  children: devices
                      .map((device) => Card(
                              child: ExpansionTile(
                            leading: Icon(device['active'] == true
                                ? Icons.devices
                                : Icons.phonelink_off),
                            ...
                            children: [
                              ...
                              ...((device['nfcFactors'] as List? ?? const [])
                                  .map((entry) => ListTile(
                                      leading: const Icon(Icons.badge),
                                      title:
                                          Text('NFC-Faktor: ${entry['label']}'),
                                      trailing: IconButton(
                              ...
                              ...((device['totpFactors'] as List? ?? const [])
                                  .map((entry) => ListTile(
