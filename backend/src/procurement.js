...
function registerProcurementRoutes({
  ...
  app.post('/api/procurement/:id/receipts/:receiptId/transfer', authMiddleware, requirePermission('procurement.receive'), (req, res) => {
    ...
    for (const receiptItem of receipt.items) {
      ...
      if (!source || (mapping.locationId && !locations.some((entry) => entry.id === mapping.locationId))) return res.status(400).json({ error: 'Der angegebene Standort ist ungültig.' });
      ...
      if (wardrobe) {
        ...
        for (let index = 0; index < count; index += 1) {
          const item = { id: nextId('clothing', clothingItems), inventoryNumber: nextClothingInventoryNumber(clothingCategoryId), name: source.name, categoryId: clothingCategoryId, size, locationId: mapping.locationId || null, stockStructureId: mapping.stockStructureId || null, status: 'Lagernd', assignedPerson: null, manufacturer: String(mapping.manufacturer || ''), manufacturingYear: String(mapping.manufacturingYear || ''), purchaseDate: receipt.receivedAt, purchasePrice: purchaseUnitPrice, inspectionIntervalMonths, lastInspectionDate: null, nextInspectionDate: addMonths(receipt.receivedAt, inspectionIntervalMonths), createdAt: now() };
          clothingItems.push(item); created.push({ entity: 'clothing', ...item });
        }
      } else if (mapping.itemType === 'bulk') {
        const item = { id: nextId('material', [...materials, ...deletedMaterials]), inventoryNumber: nextMaterialNumber(source.categoryId, source.subcategoryId), name: source.name, categoryCode: source.categoryId, subcategoryCode: source.subcategoryId || '', locationId: mapping.locationId || null, stockStructureId: mapping.stockStructureId || null, status: 'Lagernd', itemType: 'bulk', quantity: receiptItem.quantity, issuedQuantity: 0, unit: source.unit, manufacturer: String(mapping.manufacturer || ''), model: String(mapping.model || ''), serialNumber: '', purchaseDate: receipt.receivedAt, purchasePrice: purchaseUnitPrice, department: request.department, inspectionIntervalMonths: Number(mapping.inspectionIntervalMonths) || null, archived: false, createdAt: now() };
        item.manufacturingYear = String(mapping.manufacturingYear || '');
        materials.push(item); created.push({ entity: 'material', ...item });
      } else {
        const count = Math.max(1, Math.floor(receiptItem.quantity));
        for (let index = 0; index < count; index += 1) {
          const item = { id: nextId('material', [...materials, ...deletedMaterials]), inventoryNumber: nextMaterialNumber(source.categoryId, source.subcategoryId), name: source.name, categoryCode: source.categoryId, subcategoryCode: source.subcategoryId || '', locationId: mapping.locationId || null, stockStructureId: mapping.stockStructureId || null, status: 'Lagernd', itemType: 'individual', quantity: 1, issuedQuantity: 0, unit: source.unit, manufacturer: String(mapping.manufacturer || ''), model: String(mapping.model || ''), serialNumber: String((mapping.serialNumbers || [])[index] || ''), purchaseDate: receipt.receivedAt, purchasePrice: purchaseUnitPrice, department: request.department, inspectionIntervalMonths: Number(mapping.inspectionIntervalMonths) || null, archived: false, createdAt: now() };
          item.manufacturingYear = String(mapping.manufacturingYear || '');
          materials.push(item); created.push({ entity: 'material', ...item });
        }
      }
